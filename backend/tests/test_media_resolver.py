...
def test_text_block_passes_through_unchanged():
    ...
    out = asyncio.run(inline_image_urls(blocks))
    assert out == blocks


def test_external_url_passes_through_unchanged():
...
def test_same_origin_absolute_url_is_also_inlined(monkeypatch):
...
def test_fetch_failure_passes_block_through(monkeypatch):
    ...
    out = asyncio.run(inline_image_urls([original]))

    assert out[0].image_url.url == "/api/v1/media/unreachable"


def test_inline_chat_message_media_skips_str_content():
    """Plain text messages don't need media inlining."""
    msgs = [ChatMessage(role="user", content="hello")]
    out = asyncio.run(inline_chat_message_media(msgs))
    assert out == msgs
...
def test_inline_chat_message_media_does_not_mutate_input(monkeypatch):
    ...
    asyncio.run(inline_chat_message_media([msg]))
